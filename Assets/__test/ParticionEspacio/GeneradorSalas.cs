...
public class GeneradorSalas : MonoBehaviour
{
    [SerializeField] SalasNivel salasNivel;

    [SerializeField] Sala prefabSala;
    ...
    private void colocarSalas()
    {
        ...
        {
            Sala sala = Instantiate(prefabSala, casillasOcupadas[i].getCentro(), Quaternion.identity);
            sala.transform.localScale = casillasOcupadas[i].getSize();
        }
    }
    ...
    private void generarReticula()
    {
        ...
        {
            float anchura = Random.Range(salasNivel.anchraMinSala, salasNivel.anchuraMaxSala) + salasNivel.espacioEntreSalas;
